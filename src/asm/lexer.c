...
static void beginExpansion(struct String const *str, bool owned, struct String const *name)
{
...
static struct String const *readMacroArg(char name)
{
	struct String const *str = NULL;
...
static int peekInternal(uint8_t distance)
{
	for (struct Expansion *exp = lexerState->expansions; exp; exp = exp->parent) {
		...
		if (distance < exp->size - exp->offset)
			return str_Index(exp->contents.unowned, exp->offset + distance);
...
restart:
	...
	if (c == '\\' && !lexerState->disableMacroArgs) {
		...
		if (isMacroChar(c)) {
			shiftChar();
			shiftChar();
			struct String const *str = readMacroArg(c);
			...
			if (startsIdentifier(c)) {
				...
				if (tokenType == T_ID && lexerState->expandStrings) {
					...
					if (sym && sym->type == SYM_EQUS) {
						struct String const *s = sym_GetStringValue(sym);
